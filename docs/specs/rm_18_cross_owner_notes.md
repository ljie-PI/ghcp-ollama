...
- `src/cli/control_client.ts`, `src/cli/output.ts`, and `src/cli/parser.ts`: split the RM-18 CLI implementation from `src/cli/main.ts` to keep parsing, output, and control transport testable without adding public commands or default package entrypoints.
