...
describe("RM-22 atomic package cutover", () => {
  it("promotes the production package identity and entrypoints", async () => {
  ...
  it("uses TypeScript defaults and keeps SDK suites manual", async () => {
    ...
    expect(Object.keys(pkg.scripts).some((name) => name.includes("refactor") || name.includes("legacy"))).toBe(false);
  ...
  it("removes transitional configs, legacy sources, dependencies, and aliases", async () => {
    const pkg = await readPackageJson();
